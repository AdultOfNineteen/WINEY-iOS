...
@Reducer
public struct SpecificWineTastingNotes {
  ...
  public var body: some Reducer<State, Action> {
    Reduce<State, Action> { state, action in
      ...
      case ._viewWillAppear:
        ...
        return .run { send in
          switch await noteService.loadNotes(searchPage, searchSize, sortState, countries, types, buyAgain, wineId) {
      ...
      case let .tappedTastingNoteCell(noteId):
        state.noteDetail = .init(noteMode: .mynote, noteId: noteId, country: "")
    ...
    .ifLet(\.noteDetail, action: \.noteDetail) {
      NoteDetail()
    }
  }
}
